...
def DLDFGS(ds,vcl,depth):
    path_cost=0
    Frontier=[[0,path_cost,vcl[0],vcl[1]]]
    Result=[]
    Closed=[]
    
    DLDFGS_Visit(ds,vcl,depth,Frontier,Result,Closed)
    return Result
...
def DLDFGS_Visit(ds,vcl,depth,Frontier,Result,Closed):
    Index=[]
    ...
    if len(Frontier)==0:
        ...
        for i in range(len(Action[1:])):
            ...
            if node not in Closed:
                Closed.append(node)
                Frontier.insert(0,[node,path_cost,vclytmp,vclxtmp])
